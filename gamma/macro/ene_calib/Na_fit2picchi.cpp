void Na_fit2picchi()
{
 ...
 // File di input dati
    fname = "Spettro_energia_ravvicinato_delay_ampl_0deg_Na_100s"; //cambiare questo, senza .mca
  ...
  bool bkg_lineal = true;  // true per lineare, false per esponenziale
  ...
  // dichiarare histo
  TH1F* hist=new TH1F("Histo1","Spettro globale del ^{22}Na",nchan/ngroup,0,nchan);
  TH1F* peak=new TH1F("Histo2","Fit dei fotopicchi da 511 keV e 1275 keV del ^{22}Na",nchan/ngroup_peak,0,nchan);
 ...
 //Dichiarazione funzioni: retta/esponenziale e due gaussiane
  TF1 *bkg;
  if (bkg_lineal)
  ...
  TF1 *sigmoid = new TF1("sigmoid", "([0]/(1+ TMath::Exp(-[1]*(x-[2]))))", sig_inf, sig_sup);
  sigmoid->SetParameters(50, -0.5, 1400);
  
// Funzione totale come somma delle altre tre
  TF1 *total = bkg_lineal? new TF1("total","pol1+gaus(2)+gaus(5)+([8]/(1+ TMath::Exp(-[9]*(x-[10]))))",total_ini,total_fin) :  // con fondo lineal
                           new TF1("total","expo+gaus(2)+gaus(5)+([8]/(1+ TMath::Exp(-[9]*(x-[10]))))",total_ini,total_fin) ;  // con fondo exponencial
  total->SetParNames("Intercetta","Pendenza",
		     "Ampiezza 1","Centroide 1","Sigma 1","Ampiezza 2","Centroide 2","Sigma 2",
         "Capacita'", "Flesso", "Spostamento");
 ...
 // Prima approssimazione: fit del fondo e dei picchi separatamente
  ...
  peak->GetYaxis()->SetTitle("conteggi / 2 canali");
