void Na_511_fit1picco()
{
  ...
  // File di input dati
    fname = "Spettro_energia_delay_ampl_0deg_Na"; //cambiare questo, senza .mca
  ...
  TH1F* peak=new TH1F("Histo2","Fit del fotopicco da 511 keV del ^{22}Na",nchan/ngroup_peak,0,nchan);
  ...
  hist->GetYaxis()->SetTitle("conteggi / 2 canali");
  hist->Draw();
 ...
 TF1 *bkg, *total;
  if (bkg_lineal){
    ...
    total = new TF1("total","pol1(0)+gaus(2)",total_ini,total_fin);
  }
  else {
    bkg = new TF1("bkg","expo",bkg_ini,bkg_fin);    // fondo esponenziale   
    // Funzione totale come somma delle anteriori
    total = new TF1("total","expo(0)+gaus(2)",total_ini,total_fin);
  }
  ...
  c2->Print(("../../plots/fit_Na_511_" + fname + ".pdf").c_str());  
}
