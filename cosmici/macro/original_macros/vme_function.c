...
/*
 * utilizza la funzione "Tests LAM. Q=1 if the LAM is enabled and present", cioe' F8 A0
 * Il risultato della lettura (bit q) puo' essere ottenuto leggendo il 15esimo bit di dataBuffer
 */
uint8_t checkLAM(int32_t Handle, uint8_t BR, uint8_t CR, uint8_t N ) {
  ...
  if ( (dataBuffer & 0x00008000) == 0x00008000 ) {
    Q = 1;
    printf( " test Q = True: %d\n", Q);
  }
  else {
    Q = 0;
    printf( " test Q = False: %d\n", Q);
