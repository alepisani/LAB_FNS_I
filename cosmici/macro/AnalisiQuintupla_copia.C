...
void AnalisiQuintupla_copia(){
    ...
    for (int i=0; i<1000; i++){
        ...
        t24.push_back((data[2][i]-data[4][i]-T21+T41)*0.24); //qua passiamo da tdc a nanosecondi
        t35.push_back((data[3][i]-data[5][i]-T31+T51)*0.24);
        TDC1->Fill(data[1][i]);
        TDC2->Fill(data[2][i]);
        TDC3->Fill(data[3][i]);
        TDC4->Fill(data[4][i]);
    ...
    for(int i=0; i<1000; i++){
        A.push_back((3./20)*((t24[i]+t35[i])/10));  //c'è il 10 perchè ho ns
    ...
    vector<double> theta; 
    vector<double> phi;
    TH1D* htheta = new TH1D("theta", "distribuzione zenitale",200, 0., 91);
    htheta->GetYaxis()->SetRangeUser(0, 250);
    TH1D* hphi = new TH1D("phi", "distribuzione azimutale", 200, 0., 7.);
    hphi->GetYaxis()->SetRangeUser(0,70);

    for(int i=0; i<1000; i++){
        theta.push_back((asin(sqrt(A[i]*A[i]+B[i]*B[i])))*180/M_PI);
        //phi.push_back(atan2(B[i],A[i]));
